...
class ConsultarSaldo extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Bolsa de saldo', style: TextStyle(color: Colors.white))
      ),
      body: Stack(
        children: <Widget>[
  ...
  Widget _areaTrabajo(BuildContext context){
    ...
    return Container(
      ...
      child: FutureBuilder(
        future: providerSaldo.obtenerSaldo(),
          ...
          if(snapshot.hasData){
            return Column(
              children: <Widget>[
                ...
                Text('Tiempo Aire', style: GoogleFonts.pressStart2p(color: Colors.black, fontSize: 25.0, fontWeight: FontWeight.bold)),
                SizedBox(height: 25.0),
                _saldoActual(snapshot.data),
                //SizedBox(height: 25.0),
                //Divider()
              ],
          );
  ...
  Widget _saldoActual(double context){
    return Row(
