...
class RecargaTelcel extends StatefulWidget {
  @override
  _RecargaTelcelState createState() => _RecargaTelcelState();
}

class _RecargaTelcelState extends State<RecargaTelcel> {
          ...
          child: Column(
            children: <Widget>[
              ...
              Container(
                ...
                child: Image(image: AssetImage('assets/img/telcel.png'), fit: BoxFit.cover),
  ...
  _saveDatos(BuildContext context) async {
    ...
    if(num1 == num2){
      ...
      while(!recargando){
        ...
        //aqui la recarga se hizo de forma exitosa y se agrego a la base de datos entonces en esta 
        //parte tambien tenemos que agregar el descuento que le toca a la parte del saldo

        recargando = await recargasProvider.agregarRecarga(recargasModel);

        //el saldo va a disminuir ya que acabamos de vender una recarga
  ...
  Widget _ponerSubtitulo(){
    return Container(
      margin: EdgeInsets.symmetric(horizontal: 110.0, vertical: 90.0),
      child: Text('==Venta Telcel==', style: GoogleFonts.benchNine(
        color: Colors.white,
        fontSize: 30.0,
        fontWeight: FontWeight.bold
      ))
